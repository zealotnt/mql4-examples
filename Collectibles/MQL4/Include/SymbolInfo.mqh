...
//+------------------------------------------------------------------+
enum ENUM_SYMBOL_CALC_MODE  { SYMBOL_CALC_MODE_FOREX=0, SYMBOL_CALC_MODE_FUTURES=2, SYMBOL_CALC_MODE_CFD=1, SYMBOL_CALC_MODE_CFDINDEX=3, SYMBOL_CALC_MODE_CFDLEVERAGE=4 };
enum ENUM_SYMBOL_SWAP_MODE  { SYMBOL_SWAP_MODE_DISABLED,SYMBOL_SWAP_MODE_POINTS,SYMBOL_SWAP_MODE_CURRENCY_SYMBOL,SYMBOL_SWAP_MODE_CURRENCY_MARGIN,
                              SYMBOL_SWAP_MODE_CURRENCY_DEPOSIT,SYMBOL_SWAP_MODE_INTEREST_CURRENT,SYMBOL_SWAP_MODE_INTEREST_OPEN,SYMBOL_SWAP_MODE_REOPEN_CURRENT,SYMBOL_SWAP_MODE_REOPEN_BID
                            };
...
public:
   ...
   double            MarginCalc(void)                   const;
   int               Leverage(void)                     const;
   ...
   double            TickValue(void)                    const { return(SymbolInfoDouble(m_name,SYMBOL_TRADE_TICK_VALUE));}
...
//---
   ...
   if(m_tick_size==0)
      m_tick_size=MarketInfo(Name(),MODE_TICKSIZE);
//Print(m_tick_size);
   ...
   if(!Select())
     {
      if(GetLastError()!=0)
        {
         printf(__FUNCTION__+": Unknown symbol '%s'",m_name+" error ",GetLastError());
         return(false);
        }
      if(!Select(true))
        {
...
//| and adds symbol to the MarketWatch, if necessary                 |
//+------------------------------------------------------------------+
double CSymbolInfo::MarginCalc(void) const
  {
   return (MarketInfo(m_name,MODE_MARGINREQUIRED));
  }
//+------------------------------------------------------------------+
//| Checks if symbol is selected in the MarketWatch                  |
//| and adds symbol to the MarketWatch, if necessary                 |
//+------------------------------------------------------------------+
int CSymbolInfo::Leverage(void) const
  {
   int leverage = 0;
   if(MarginCalc() > 0)
      leverage = (int) MathRound(ContractSize() * Bid() * convert(1.0,SymbolInfoString(m_name,SYMBOL_CURRENCY_PROFIT),AccountCurrency()) / MarginCalc());
   return(leverage);
  }
//+------------------------------------------------------------------+
//| Checks if symbol is selected in the MarketWatch                  |
//| and adds symbol to the MarketWatch, if necessary                 |
//+------------------------------------------------------------------+
double convert(double amount, string fromCcy, string toCcy)
  {
   double base_bid=0, base_margin=0;
   if(toCcy == fromCcy)
      base_margin = amount;
   else
      if(SymbolInfoDouble(fromCcy+toCcy, SYMBOL_BID, base_bid))
         base_margin = amount * base_bid;
      else
         if(SymbolInfoDouble(toCcy+fromCcy,SYMBOL_BID,base_bid))
            if(base_bid!=0)
               base_margin = amount / base_bid;
            else
               return(0);
         else
            if(SymbolInfoDouble(toCcy+"USD",SYMBOL_BID,base_bid)&&base_bid>0)
               if(SymbolInfoDouble(fromCcy+"USD",SYMBOL_BID,base_bid)&&base_bid>0)
                  base_margin = amount * SymbolInfoDouble(fromCcy+"USD",SYMBOL_BID) / SymbolInfoDouble(toCcy+"USD",SYMBOL_BID);
               else
                  if(SymbolInfoDouble("USD"+fromCcy,SYMBOL_BID) > 0)
                     base_margin = amount / SymbolInfoDouble("USD"+fromCcy,SYMBOL_BID) / SymbolInfoDouble(toCcy+"USD",SYMBOL_BID);
                  else
                     base_margin = 0;
            else
               if(SymbolInfoDouble(fromCcy+"USD",SYMBOL_BID,base_bid))
                  base_margin = amount * SymbolInfoDouble(fromCcy+"USD",SYMBOL_BID) * SymbolInfoDouble("USD"+toCcy,SYMBOL_BID);
               else
                  if(SymbolInfoDouble("USD"+fromCcy,SYMBOL_BID) > 0)
                     base_margin = amount / SymbolInfoDouble("USD"+fromCcy,SYMBOL_BID) * SymbolInfoDouble("USD"+toCcy,SYMBOL_BID);
                  else
                     base_margin = 0;
   return(base_margin);
  }
